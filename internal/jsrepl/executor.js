...
// in). Long-polls /jsin, evals jobs in the host page's global scope,
// posts results to /stdout. No UI — drive everything from the laptop.
(function () {
  ...
  function post(out, cb) {
    ...
    p.open("POST", BASE + "/stdout", true);
    p.setRequestHeader("Content-Type", "application/json");
    p.onreadystatechange = function () { if (p.readyState === 4) cb(); };
    p.onerror = function () { cb(); };
    try { p.send(JSON.stringify(out)); } catch (_) { cb(); }
  }

  function tick() {
    var x = new XMLHttpRequest();
    x.open("GET", BASE + "/jsin", true);
    x.onreadystatechange = function () {
      if (x.readyState !== 4) return;
      if (x.status === 200 && x.responseText) {
        var job, out;
        try { job = JSON.parse(x.responseText); }
        ...
        post(out, function () { setTimeout(tick, 0); });
    ...
    x.onerror = function () { setTimeout(tick, 3000); };
    try { x.send(); } catch (_) { setTimeout(tick, 3000); }
  }
